...
class Survey:
    ...
    def inRegion(self, pulsar):
        ...
        if abs(gb) > self.GBmax or abs(gb) < self.GBmin:
            return False
        
        ra, dec = go.lb_to_radec(gl, gb)
    ...
    def inPointing(self, pulsar):
        ...
        return offset_deg
##############################################################################  
    def inPointing_vectorized(self, pulsars):

        gl = np.array([p.gl for p in pulsars])
        gb = np.array([p.gb for p in pulsars])
    
        coords = np.column_stack((gl, gb))
    
        # pointingslist only contains gl, gb
        pcoords = self.pointingslist
    
        #from scipy.spatial import cKDTree
    
        tree = cKDTree(pcoords)
    
        offset, closest = tree.query(coords)
...
#####################################################################################################################################################3
    def SNRcalc_new(self,
        ...
        period = np.array([p.period for p in pulsars])
        dm = np.array([p.dm for p in pulsars])
        gl = np.array([p.gl for p in pulsars])
        gb = np.array([p.gb for p in pulsars])
        width_deg = np.array([p.width_degree for p in pulsars])
        scindex = np.array([p.scindex for p in pulsars])
    
        binary = np.array([p.is_binary for p in pulsars])
        ...
        inside = (
            (gl >= self.GLmin) &
            (gl <= self.GLmax) &
            (np.abs(gb) >= self.GBmin) &
            (np.abs(gb) <= self.GBmax)
        )
        ...
        # Pointing / beam check
        # -----------------------------
        gain = np.full(len(idx), self.gain)
        tobs = np.full(len(idx), self.tobs)
    
        if self.pointingslist is not None:
    
            offset, gain, tobs = self.inPointing_vectorized(
                np.array(pulsars, dtype=object)[idx]
            )
    
            # offset 
            matched = offset < self.fwhm
    
            # remove pulsars outside beam
            idx = idx[matched]
            offset = offset[matched]* 60.0
            gain = gain[matched]
            tobs = tobs[matched]
            ...
            offset = (
                self.fwhm *
                np.sqrt(np.random.random(len(idx))) /
                2.0
            )
        ...
        if self.gainpat == 'airy':
            ...
            eff_diam = (
                3.0e8 /
                (self.freq*self.fwhm*conv*1.0e6)
            )
    
            a = eff_diam/2.
    
            lamda = 3.0e8/(self.freq*1.0e6)
        ...
        tdm = self._dmsmear_vectorized(dm[idx])
        
        tscat = go.scatter_bhat_vectorized(
            dm[idx],
            scindex[idx],
            self.freq
        )
        ...
        duty = weff_ms / period[idx]
        
        
        
        # Mark smeared pulsars
        smeared = duty > 1.0
        
        snr[idx[smeared]] = -1.0
        
        
        # Keep only usable pulsars
        good = ~smeared
        ...
        if len(good_idx) > 0:
            ...
            good_pulsars = np.array(
                pulsars,
                dtype=object
            )[good_idx]
        
        
            flux = self.calcflux_vectorized(
                good_pulsars,
                pop.ref_freq
            )
        
        
            tsky = self.tskypy_vectorized(
                good_pulsars
            )
        
        
            snr[good_idx] = rad.calcSNR_vectorized(
                flux,
                self.beta,
                self.tsys,
                tsky,
        ...
        # Apply beam attenuation only to good pulsars
        snr[good_idx] *= degfac[good]
...
##########################################################################################################
    def calcflux_vectorized(self, pulsars, ref_freq):

        gpsFlag = np.array([p.gpsFlag for p in pulsars])
        brokenFlag = np.array([p.brokenFlag for p in pulsars])
    
        spindex = np.array([p.spindex for p in pulsars])
        brokenSI = np.array([p.brokenSI for p in pulsars])
    
        # Calculate S1400
        s1400 = np.array( [p.s_1400() for p in pulsars] )
    
        flux = np.zeros(len(pulsars))
    
        # Normal spectral index
        normal = (
            (gpsFlag == 0) &
            ~(
                (brokenFlag == 1) &
        ...
        flux[normal] = (
            s1400[normal] *
            (self.freq/ref_freq)**spindex[normal]
        )
        ...
        flux[broken] = (
            s1400[broken] *
            (self.freq/ref_freq)**brokenSI[broken]
        )
        ...
        if np.any(gps):
            ...
            flux[gps] = self._gpsFlux_vectorized(
                np.array(pulsars,dtype=object)[gps],
                ref_freq
            )
...
######################################################################################################3
   ...
   ##################################################################################################
    def _dmsmear_vectorized(self, dm):

        return (8.3E6 *dm *self.bw_chan /self.freq**3)
########################################################################################################
    def tskypy(self, psr):
    ...
    def tskypy_vectorized(self, pulsars):
        """Calculate sky temperature from Haslam table for an array of pulsars."""

        gl = np.array([p.gl for p in pulsars])
        gb = np.array([p.gb for p in pulsars])
    
        # ensure longitude is 0 -> 360
        l = np.where(gl < 0, 360 + gl, gl)
    
        # convert to Haslam table indices
        j = gb + 90.5
        j = np.clip(j, 0, 179)
        ...
        tsky_haslam = np.asarray(self.tskylist)[index]
