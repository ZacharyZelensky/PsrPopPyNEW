...
def tm98_fraction(period):
    """
    Tauris & Manchester 1998 beaming fraction.
    ...
    -------
    fraction : ndarray
        Beaming fraction for each pulsar.
    """
    periodterm = np.log10(period / 1000.0) - 1.0
...
def wj08_fraction(period, chi):
    """
    Weltevrede & Johnston 2008 beaming fraction.
    ...
    # Small inclination angles treated as zero
    chi = np.where(chi < 1.0E-5, 0.0, chi)

    # Beam radius in degrees
    rho = 5.4 * (period / 1000.0)**(-0.5)
    ...
    # Conditions
    cond1 = (chi > rho) & ((chi + rho) < 90.0)
    cond2 = (chi > rho) & ((chi + rho) > 90.0)
    cond3 = (chi <= rho) & ((chi + rho) < 90.0)

    # Default case
    fraction = np.ones_like(period, dtype=float)

    # chi > rho, chi + rho < 90
    fraction = np.where(
        cond1,
        2.0 * np.sin(chi_rad) * np.sin(rho_rad),
        fraction
    )
